...
\subsubsection*{Praha}
\begin{center}
<<Grafy_hist_pha, fig=TRUE, echo=false , height=4, width=5>>=
...
\begin{center}
<<Grafy_hist_vys, fig=TRUE, echo=false , height=4, width=5>>=
...
\begin{center}
<<Grafy_hist_pce, fig=TRUE, echo=false , height=4, width=5>>=
...
Obličejové grafy slouží k zobrazení vícerozměrných dat. K tomu využívají schopnost člověka rozpoznávat a hodnotit rozdíly mezi lidskými tvářemi. Každý objekt je reprezentován schematickým obličejem, ve kterém tvar či velikost jednotlivých rysů (délka nosu, tvar úst, sklon obočí, šířka tváře) představují hodnotu odpovídajícího atributu. Původní Chernoffův návrh mohl zobrazit až 18 atributů.\\[-1\baselineskip]
\begin{center}
...
\begin{quote}
\emph{Zařadil jsem ho sem pouze pro zajímavost. Nezabýval jsem se jeho napojením na data a tedy grafy nejspíše nereflektují realitu.}
\end{quote}
\newpage
...
Kovariance může nabývat hodnot na intervalu $\langle-\infty, +\infty\rangle$. Na základě vypočtené kovariance můžeme posoudit vzájemnou závislost následujícím způsobem:
...
\begin{quote}
\emph{Při vzájemném porovnání všech krajů velmi snadno zjistíme, že data jsou vzájemně opravdu silně závislá. Lze tedy tvrdit, že pokud poroste cena v jednom kraji, velmi praděpodbně poroste cena i v kraji druhém.}
\end{quote}
...
Při použití vzorce získáme data:
<<>>=
cov(Pha_mtx, Vys_mtx) / (sqrt(var(Pha_mtx) * var(Vys_mtx)));
@
...
V matici vidíme, že korelační koeficieent je stejný jako u výše použitého vrozce.

\begin{quote}
\emph{Při pohledu na korelační matici snadno dojdeme k závěru, že jsou na sobě všechny relativně silně závislé. můžeme tím tedy říci, že jsou KORELOVANÉ.}
\end{quote}

\subsection*{Empirická komulativní distribuční funkce}
Distribuční funkce nebo lidově ``kumulovaná pravděpodobnost'' je funkce, která udává pravděpodobnost, že hodnota náhodné proměnné je menší než zadaná hodnota.
Empirická distribuční funkce však slouží jako odhad skutečné distribuční funkce náhodné veličiny. 

\begin{center}
<<Grafy_distribucni_funkce_pce, fig=true, results=hide>>=
...
\subsubsection*{Statistická hypotéza}
Jako statistickou hypotézu chápeme určitý předpoklad o rozdělení náhodných veličin.
\subsubsection*{Statistický test}
Při testování statistických hypotéz se vždy porovnávají dvě hypotézy. První hypotéza, nulová, je hypotéza, která se testuje; značí se obvykle $H_{0}$. Druhou hypotézou je alternativní hypotéza, obvykle značená $H_{1}$.

\subsubsection*{Testování hypotézy o cennách}
Na samotný začátek musíme vyslovih hypotézu, neboli náš předpoklad, který chceme testovat.
\begin{center}
\texttt{\emph{Průměrné spotřebitelské ceny hovězího masa v Praze (x) jsou vyšší než na Vysočině (y).}}
\end{center} 

\noindent Dále formulujeme hypotézy $H_{0}$ a $H_{1}$:

\begin{center}
$H_{0}:~~\bar{x}\leq\bar{y}$
...
\noindent Máme vypočítanou hodnotu $ttestu$, která vyšla \texttt{$\Sexpr{T}$}. Nyní v tabulkách vyhledáme kritickou hodnotu pro \texttt{$\Sexpr{pocet+pocet-2}$} stupňů volnosti s maximální přípustnou chybou \texttt{5\%}.

\begin{center}
$t_{0.975} = \Sexpr{t_0975}$
\end{center}

\noindent V předposledním kroku vytvoříme interval, jehož pomocí rozhodneme, zda zamítneme hypotézu $H_{0}$, či nikoliv.

\begin{center}
$W = (-\infty; -\Sexpr{t_0975}) \cup (\Sexpr{t_0975};\infty)$
\end{center}
...
\\[0.75\baselineskip]
\texttt{ {\Large \textcolor{red}{Nic však nepotvrzujeme!}}}
\end{center}
...
Tentokrát k celému výpočtu použijeme funkci \texttt{t.test(x,y)}, která celý výpočet provede zcela automaticky a za nás. Jen si musíme dám pozor na výstupní data. Budeme vycházet z předchozího příkladu a použijeme již definované hypotézy:
...
\\[0.75\baselineskip]
\texttt{ {\Large \textcolor{red}{Opět nic nepotvrzujeme!}}}
\end{center}
...
<<aov_data>>=
aov(Cena ~ Rok * Tyden * Kraj, data_aov);
@
